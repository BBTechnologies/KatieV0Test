import React from 'react';
import type { Meta, StoryObj } from '@storybook/react';
import { PopupMenu } from './PopupMenu';
import { MenuMock } from '../../../mocks/mainMenu.mock';
import { UserPicButton } from '../../form/buttons/userPicButton/UserPicButton';

const meta: Meta<typeof PopupMenu> = {
  title: 'Navigation/PopupMenu',
  component: PopupMenu,
  decorators: [
    (Story, context) => (
      <div className="popupMenuDemo">
        <p>Popup menus can pop in all directions and will try to float over the top of the elements around them.</p>
        <p>For the purposes of testing and demonstration the popup triggerSettings has been placed in the center of a div.</p>
        <p>The container also has lots of padding top and bottom to accommodate the height of the menu.</p>

        <div className="demoBar">
          <Story { ...context } />
        </div>
        <p>This is content that appears underneath the popup menu.</p>
        <p>The popup toggles open and closed via the triggerSettings button.</p>
        <p>It will also close if you click anywhere outside the bounds of the component or hit the escape key.</p>
      </div>
    )
  ]
};

export default meta;

type Story = StoryObj<typeof PopupMenu>;

export const SimplePopupMenu: Story = {
  args: {
    triggerSettings: {
      default: {
        leftIcon: { icon: 'happy' },
        rightIcon: { icon: 'chevronDownSelect' },
        label: 'Happy popup menu',
        id: 'demoPopupMenutriggerSettings'
      }
    },
    menuItems: MenuMock
  }
};

export const SingleIcontriggerSettings: Story = {
  args: {
    triggerSettings: {
      default: {
        leftIcon: { icon: 'happy' },
        label: 'Happy popup menu',
        hideLabel: true,
        id: 'demoPopupMenutriggerSettings2'
      }
    },
    menuItems: MenuMock
  }
};

export const PopsLeftAndDown: Story = {
  args: {
    triggerSettings: {
      default: {
        leftIcon: { icon: 'happy' },
        label: 'Happy popup menu',
        hideLabel: true,
        id: 'demoPopupMenutriggerSettings3'
      }
    },
    popDirection: {
      x: 'left'
    },
    menuItems: MenuMock
  }
};

export const PopsLeftAndUp: Story = {
  args: {
    triggerSettings: {
      default: {
        leftIcon: { icon: 'happy' },
        label: 'Happy popup menu',
        hideLabel: true,
        id: 'demoPopupMenutriggerSettings4'
      }
    },
    popDirection: {
      x: 'left',
      y: 'up'
    },
    menuItems: MenuMock
  }
};

export const PopsRightAndUp: Story = {
  args: {
    triggerSettings: {
      default: {
        leftIcon: { icon: 'happy' },
        label: 'Happy popup menu',
        hideLabel: true,
        id: 'demoPopupMenutriggerSettings5'
      }
    },
    popDirection: {
      y: 'up'
    },
    menuItems: MenuMock
  }
};

export const UserPicAstriggerSettings: Story = {
  args: {
    triggerSettings: {
      userSettings: {
        id: 'BabySlothPopupMenu',
        label: 'Baby Sloth Popup Menu',
        user: {
          imageUrl: 'data:image/jpeg;base64,',
          fullName: 'Baby Sloth'
        }
      }
    },
    menuItems: MenuMock
  }
};

export const NotificationsTriggerSettings: Story = {
  args: {
    triggerSettings: {
      notificationsSettings: {
        id: 'NotificationsPopupMenu',
        label: 'Notifications Popup Menu',
        count: 12
      }
    },
    menuItems: MenuMock
  }
};
