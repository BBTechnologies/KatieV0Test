...
describe('Select', () => {
  ...
  const CreateTestField = (customTestProps: FormFieldProps = testProps) => {
    ...
    return {
      ...
      field: container.querySelectorAll('select')[0]
    };
  };
  ...
  it('should register the select into the react-hook-form provider context', () => {
    CreateTestField();
    ...
    describe('Select label', () => {
      ...
      describe('Label dom structure', () => {
        it('The label tag wraps the input creating an implicit relationship for accessibility', () => {
          const { label } = CreateTestField();
          expect(label.querySelectorAll('select')).toHaveLength(1);
        });
        ...
        it('The span.labelText is the first child of the span by default', () => {
          const { label } = CreateTestField({
            ...testProps,
